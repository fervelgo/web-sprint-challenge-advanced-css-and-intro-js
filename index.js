...
function get20s(array){
  ...
  for (let i=0; i < array.length; i++){
    ...
    if (array[i].years[0].includes("19") && array[i].years[2].includes("19")) {
...
function addArtist(array){
  let newArtist = { 
    ...
    "genre": "Web Design", 
    "nationality": "Mexican",
    "bio": "Born and raised in CDMX. Interested in tech and almost everything else, basically",
  }
...
function lotsOfArt(array){
  ...
  for (let i = 0; i < array.length; i++) {
    if (array[i].paintings > 100) {
      plus100Artists.push(array[i].name)
    }
  }
  return plus100Artists
}
